...
They express domain meaning, indicate invariant violations, and are not technical
failures. Unlike application errors (which signal infrastructure problems), domain
errors describe *why* something is invalid in domain terms. All entity errors protect
the identity lifecycle — IDs are assigned once, never deleted, and never modified.
They extend the [Foundation](../foundation.md) `Error` base class.
