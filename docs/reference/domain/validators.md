...
Concrete `ValidationRule` implementations for common input constraints. Each validator is a stateless, reusable rule that returns `RuleViolatedError` instances when the input fails its constraint.
...
```python
RequiredValidator("username").validate(None)     # -> [RuleViolatedError]
...
EmailValidator("email").validate("not-an-email")      # -> [RuleViolatedError]
```
...
```python
LengthValidator("password", minimum_length=8, maximum_length=128).validate("short")  # -> [RuleViolatedError]
...
```python
RangeValidator("age", minimum_value=0, maximum_value=150).validate(-5)  # -> [RuleViolatedError]
...
composite.validate("bad")  # -> [RuleViolatedError for invalid_email]
```

## When to use

Use individual validators for single-field checks in commands, queries, and API input. Compose them with `CompositeValidationRule` when multiple constraints apply to the same field or when several fields must be validated together. All validators use the foundation `RuleViolatedError` type with `ErrorMetadata` carrying field and error-code context.

!!! note "Related"
    See [Foundation Rules](../foundation/rules.md) for the `ValidationRule` base class and [Foundation Errors](../foundation/errors.md) for `RuleViolatedError`.
